document.addEventListener("DOMContentLoaded", () => {
    ...
    const output = document.getElementById("purchase_ex_rate");
    const currencyInput = document.getElementById("currency");

    function fetchEX(handle) {
        const apiUrl = `https://nbg.gov.ge/gw/api/ct/monetarypolicy/currencies/ka/json/?date=${dateInput.value}`;

        fetch(apiUrl)
            .then((response) => response.json())
            .then((data) => {
                // if RUB is selected recalculate currency
                if (currencyInput.value === "RUB") {
                    output.value = data[0]["currencies"].find(
                        (currency) => currency.code === currencyInput.value
                    ).rateFormated / 100;
                } else {
            ...
            .catch((error) => {
                output.value = "An error occurred while fetching data.";
                console.error(error);
            });
    }

    dateInput.addEventListener("change", fetchEX);
    currencyInput.addEventListener("change", fetchEX);
