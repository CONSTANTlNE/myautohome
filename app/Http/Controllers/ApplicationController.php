...
//dd($request->status);
        ...
        if ($app->car_id != $request->car) {
            ...
            Log::channel('changes')->info('განაცხადში No: '.$app->id.'  შეიცვალა მანქანა: -- '.$app->car->make.' -- ახალი მანქანაა: -- '.$cars->where('id',
        ...
        foreach ($request['company'] as $company) {
            if (!in_array($company, $app->companies->pluck('id')->toArray())) {
                Log::channel('changes')->info('განაცხადში No: '.$app->id.'  ჩაემატა კომპანია: '.$companies->where('id',
